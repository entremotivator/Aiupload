...
def get_vapi_config():
    """Get Vapi API configuration from secrets or environment"""
    try:
        # Try to get from Streamlit secrets first
        if hasattr(st, 'secrets') and 'vapi' in st.secrets:
            return {
                'api_key': st.secrets.vapi.get('api_key', ''),
                'phone_number_id': st.secrets.vapi.get('phone_number_id', '431f1dc9-4888-41e6-933c-4fa2e97d34d6'),
            ...
            return {
                ...
                'phone_number_id': os.getenv('VAPI_PHONE_NUMBER_ID', '431f1dc9-4888-41e6-933c-4fa2e97d34d6'),
                'base_url': os.getenv('VAPI_BASE_URL', 'https://api.vapi.ai')
            }
    except Exception as e:
        st.error(f"Error loading Vapi configuration: {str(e)}")
        return {
            'api_key': '',
            'phone_number_id': '431f1dc9-4888-41e6-933c-4fa2e97d34d6',
