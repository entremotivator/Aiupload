...
def get_global_client():
    """Get client using global credentials"""
    global_creds = st.session_state.get("global_gsheets_creds")
    if not global_creds:
        return None
    return get_gsheet_client(global_creds)

def get_sheet_as_df(sheet_name: str, worksheet_index: int = 0) -> Optional[pd.DataFrame]:
    """Get Google Sheet as DataFrame with error handling using global credentials"""
    global_creds = st.session_state.get("global_gsheets_creds")
    if not global_creds:
        st.error("🔑 Global Google Sheets credentials not found. Please upload in sidebar.")
        return None
    
    try:
        client = get_gsheet_client(global_creds)
        if not client:
            return None
...
def update_sheet_from_df(sheet_name: str, df: pd.DataFrame, worksheet_index: int = 0) -> bool:
    """Update Google Sheet from DataFrame with validation using global credentials"""
    global_creds = st.session_state.get("global_gsheets_creds")
    if not global_creds:
        st.error("🔑 Global Google Sheets credentials not found. Please upload in sidebar.")
        return False
...
def create_new_sheet(sheet_name: str, df: pd.DataFrame) -> bool:
    """Create new Google Sheet with data using global credentials"""
    global_creds = st.session_state.get("global_gsheets_creds")
    if not global_creds:
        st.error("🔑 Global Google Sheets credentials not found. Please upload in sidebar.")
        return False
...
def list_available_sheets() -> list:
    """List all available Google Sheets using global credentials"""
    global_creds = st.session_state.get("global_gsheets_creds")
    if not global_creds:
        return []
...
def get_sheet_by_url(sheet_url: str, worksheet_index: int = 0) -> Optional[pd.DataFrame]:
    """Get Google Sheet by URL using global credentials"""
    global_creds = st.session_state.get("global_gsheets_creds")
    if not global_creds:
        st.error("🔑 Global Google Sheets credentials not found. Please upload in sidebar.")
        return None
    ...
    try:
        ...
        with st.spinner("Loading sheet from URL..."):
        ...
        st.error(f"Error loading sheet from URL: {str(e)}")
        return None
