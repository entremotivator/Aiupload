...
def logout_user():
    ...
    keys_to_clear = [
        ...
        "login_time", "session_expiry", "global_gsheets_creds", "gsheets_creds",
        "data_cache", "notifications", "sync_status", "current_page"
    ]
...
def get_global_credentials():
    """Get global Google Sheets credentials"""
    return st.session_state.get("global_gsheets_creds")

def ensure_global_credentials():
    """Ensure global credentials are available, show error if not"""
    if "global_gsheets_creds" not in st.session_state:
        ...
        st.info("💡 The same credentials will be used across all pages that need Google Sheets access.")
        return False
    return True
